...
class Rectangle(Shape):
    def __init__(self, topleft_corner: PointType, length: int, breadth: int):
        topleft_corner_point = Point(*topleft_corner)
        self.center: Point = Point(
            topleft_corner_point.x + length // 2, topleft_corner_point.y + breadth // 2
        )
        self.length = length
    ...
    def draw_on_image(
        self, image: Image.Image, color: RGBColorType, to_fill: Optional[bool] = False
    ) -> Image.Image:
        img_draw = ImageDraw.Draw(image)
        if to_fill:
            img_draw.polygon(self.get_points(), fill=color, width=2)
        else:
            img_draw.polygon(self.get_points(), outline=color, width=2)
        return image

    @classmethod
    def get_n_rectangle_rand(
        cls,
        rectangle_count: int,
        width: int,
        height: int,
