...
from stunning_wallpaper.backgroundgenerator.shapes.shape import ShapeType
...
def draw_shapes_rand(
    image: Image.Image,
    shape_list: List[ShapeType],
...
def draw_center_shapes_rand(
    image: Image.Image,
    shape_list: List[ShapeType],
    middle_shape: ShapeType,
    color_list: List[Tuple[int, int, int]],
    to_fill: bool,
    center_only: bool,
) -> Image.Image:
    for shape in shape_list:
        ...
        is_shape_inside: bool = middle_shape.is_inside_shape(
            (x1, y1)
        ) and middle_shape.is_inside_shape((x2, y2))
        if not center_only:
...
def layered_shapes_rand(
    image: Image.Image, layer_width: int, color_list: List[Tuple[int, int, int]]
) -> Image.Image:
    return image
